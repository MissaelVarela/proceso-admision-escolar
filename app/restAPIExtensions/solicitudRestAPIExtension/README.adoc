...
= Solicitud REST API Extension
API REST extension para manejar peticiones de las solicitudes de un usuario.

The project **solicitudRestAPIExtension** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/solicitudRestAPIExtension.git
cd solicitudRestAPIExtension/
mvn package
----

The build should produce a zip archive under the `target/` folder named `solicitudRestAPIExtension-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `solicitudRestAPIExtension-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
